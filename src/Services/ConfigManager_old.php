...
class ConfigManager_old
{
    ...
    protected static function validateDevTeamEmail($email)
    {
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            return null;  // Or you can decide on a specific fallback email
        }
    }
    ...
    protected static function validateRouteChannel($channel)
    {
        ...
        if (in_array($channel, $validChannels)) {
            ...
            return 'upload';  // Fallback on a default value
        }
    }
    ...
    {
        $value = config("error_manager.{$key}", $default);
